...
module type S = sig @@ portable
  ...
  (** Constructs a [t] containing only a string from a format. This eagerly constructs the
      string. *)
  ...
  (** [of_exn] and [to_exn] are primarily used with [Error], but their definitions have to
      ...
      caveats as for [Printexc.print_backtrace] apply. [~backtrace:(`This s)] attaches the
      backtrace [s]. The default is no backtrace. *)
...
module type Info = sig @@ portable
  module type S = S
  module type Internal_repr = Internal_repr
