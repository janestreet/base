module type List0 = sig
  ...
  val length : ('a t[@kind k]) -> int
  val exists : ('a t[@kind k]) -> f:('a -> bool) -> bool
  val iter : ('a t[@kind k]) -> f:('a -> unit) -> unit

  val rev_append : ('a t[@kind k]) -> ('a t[@kind k]) -> ('a t[@kind k])
  [@@alloc __ @ m = (stack_local, heap_global)]

  val rev : ('a t[@kind k]) -> ('a t[@kind k])
  [@@alloc __ @ m = (stack_local, heap_global)]

  val for_all : ('a t[@kind k]) -> f:('a -> bool) -> bool
  ...
  val rev_map : ('a t[@kind ka]) -> f:('a -> 'b) -> ('b t[@kind kb])]

  val fold2_ok : 'a t -> 'b t -> init:'c -> f:('c -> 'a -> 'b -> 'c) -> 'c
  val exists2_ok : 'a t -> 'b t -> f:('a -> 'b -> bool) -> bool
  val iter2_ok : 'a t -> 'b t -> f:('a -> 'b -> unit) -> unit
  val for_all2_ok : 'a t -> 'b t -> f:('a -> 'b -> bool) -> bool
  val nontail_map : 'a t -> f:('a -> 'b) -> 'b t
  val rev_map2_ok : 'a t -> 'b t -> f:('a -> 'b -> 'c) -> 'c t
  ...
  val fold_right2_ok : 'a t -> 'b t -> f:('a -> 'b -> 'c -> 'c) -> init:'c -> 'c
end
