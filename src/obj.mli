...
[@@ocaml.doc
  ...
  \      [reachable_words]) of all heap blocks accessible from the argument but \
   excluding all\n\
  \      blocks accessible from any other arguments.\n\n\
  \      Also returns a single number denoting the total memory reachable from at least \
   two\n\
