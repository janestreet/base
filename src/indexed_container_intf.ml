...
module Definitions = struct
  ...
  module type
    ...
    val find_mapi
      : ('a : k1) 'p1 'p2 ('b : k2).
      ('a, 'p1, 'p2) t @ mi
      -> f:(int -> 'a elt @ mi -> ('b Option0.t[@kind k2 or value_or_null]) @ mo) @ local
      -> ('b Option0.t[@kind k2 or value_or_null]) @ mo]
  ...
  include struct
    ...
    include struct
      ...
      module type Creators_with_index = sig
        ...
        type ('a : k1, 'b, 'c) t := (('a, 'b, 'c) t[@kind k1]) [@@kind.explicit k1 = ks]
        type ('a : k1) elt := ('a elt[@kind k1]) [@@kind.explicit k1 = ks]
        ...
        type ('a : k1, 'b, 'c) t := (('a, 'b, 'c) t[@kind.explicit k1']) [@@kind value]
        type ('a : k1) elt := ('a elt[@kind.explicit k1']) [@@kind value]
        ...
        val mapi
          : ('a : k1) 'p1 'p2 ('b : k2).
          (('a, 'p1, 'p2) t[@kind.explicit k1']) @ m
          -> f:
               (int
                -> ('a elt[@kind.explicit k1']) @ m
                -> ('b elt[@kind.explicit k2']) @ n)
             @ local
          -> (('b, 'p1, 'p2) t[@kind.explicit k2']) @ n
  ...
  include struct
    ...
    module type S1_with_creators = sig
      ...
      [@kind_set.explicit ks]
      [@alloc a]
      [@with: type ('a : k mod separable) t := ('a t[@kind k]) [@@kind k = ks]]
