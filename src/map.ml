...
module Tree0 = struct
  ...
  let globalize : 'k 'v. ('k, 'v) t @ local -> ('k, 'v) t = function
    | Empty -> Empty
        ...
        update acc inner_key ~compare_key:inner_comparator.Comparator.compare ~f:(function
          ...
          | Some elt ->
            set
              ...
              ~key:outer_key
              ~data
              ~compare_key:outer_comparator.Comparator.compare [@nontail])
          [@nontail]))
  ;;
...
let globalize_m__t (module _ : Globalize_m) _ (local_ t) = globalize t
