...
let range' ~compare ~stride ?(start = `inclusive) ?(stop = `exclusive) start_i stop_i =
  ...
  let initial_stride_order =
    ...
    | Greater -> `Greater
  in
  let[@tail_mod_cons] rec local_ range_loop i =
...
let init_internal n ~f ~name =
  ...
  (let rec init_loop i accum =
     assert (i >= 0);
     if i = 0 then accum else init_loop (i - 1) (f (i - 1) :: accum) [@exclave_if_stack a]
   in
   init_loop n [] [@nontail])
  [@exclave_if_stack a]
[@@alloc a = (heap, stack)]
...
let init_i64 n ~f =
  let open struct
    ...
    external ( - ) : t -> t -> t @@ portable = "%int64#_sub"
    external ( < ) : t -> t -> bool @@ portable = "%int64#_lessthan"
  end in
  (let rec init_i64_loop n acc =
     let n = n - #1L in
     if n < #0L
     then acc
     else (init_i64_loop [@tailcall]) n (f n :: acc) [@exclave_if_stack a]
   in
   init_i64_loop n [] [@nontail])
  [@exclave_if_stack a]
[@@alloc a = (heap, stack)]
;;
...
let%template[@zero_alloc] nth_or_null t n =
  if n < 0
  then Null
  else (
...
let%template check_length2 (l1 @ li) (l2 @ li) ~f @ lo =
...
let filteri l ~f =
  let[@tail_mod_cons] rec local_ filteri_loop pos l =
    match l with
    | [] -> []
    | hd :: tl ->
...
let find_map t ~f =
  (let rec find_map_loop = function
     | [] -> None
     | x :: l ->
       (match[@exclave_if_local lo ~reasons:[ May_return_local ]] f x with
        | None -> find_map_loop l
        | Some _ as r -> r)
   in
   find_map_loop t [@nontail])
  [@exclave_if_local lo ~reasons:[ May_return_local ]]
;;]

let%template find_map_or_null t ~f =
  (let rec find_map_or_null_loop = function
     | [] -> Null
     | x :: l ->
       (match[@exclave_if_local lo ~reasons:[ May_return_local ]] f x with
        | Null -> find_map_or_null_loop l
        | This _ as r -> r)
   in
   find_map_or_null_loop t [@nontail])
  [@exclave_if_local lo ~reasons:[ May_return_local ]]
[@@mode li = (global, local), lo = (global, local)]
;;

let find_map_or_null_exn =
  let not_found = Not_found_s (Atom "List.find_map_or_null_exn: not found") in
  let find_map_or_null_exn t ~f =
    match find_map_or_null t ~f with
    | Null ->
      raise (Portability_hacks.magic_uncontended__promise_deeply_immutable not_found)
    | This x -> x
  in
  (* named to preserve symbol in compiled binary *)
  find_map_or_null_exn
;;
...
let find t ~f =
  (let rec find_loop = function
     | [] -> None
     | x :: l ->
       if f x then Some x [@exclave_if_local l] else find_loop l [@exclave_if_local l]
   in
   find_loop t [@nontail])
...
let find_or_null t ~f =
  (let rec find_or_null_loop = function
     | [] -> Null
     | x :: l -> if f x then This x else find_or_null_loop l [@exclave_if_local l]
   in
   find_or_null_loop t [@nontail])
...
let findi t ~f =
  (let rec findi_loop i t =
     match[@exclave_if_local l ~reasons:[ May_return_regional; Will_return_unboxed ]]
       t
     with
     | [] -> None
     | x :: l -> if f i x then Some (i, x) else findi_loop (i + 1) l
   in
   findi_loop 0 t [@nontail])
  [@exclave_if_local l ~reasons:[ May_return_regional; Will_return_unboxed ]]
;;
  ...
  (let rec find_mapi_loop i t =
     ...
     | x :: l ->
       (match f i x with
        ...
        | None ->
          let next = i + 1 in
          find_mapi_loop next l)
   in
   find_mapi_loop 0 t [@nontail])
  [@exclave_if_local lo ~reasons:[ May_return_local ]]
;;

let find_mapi_or_null t ~f =
  (let rec find_mapi_or_null_loop i t =
     match[@exclave_if_local lo ~reasons:[ May_return_local ]] t with
     | [] -> Null
     | x :: l ->
       (match f i x with
        | This _ as result -> result
        | Null ->
          let next = i + 1 in
          find_mapi_or_null_loop next l)
   in
   find_mapi_or_null_loop 0 t [@nontail])
  [@exclave_if_local lo ~reasons:[ May_return_local ]]
;;]

let find_mapi_or_null_exn =
  let not_found = Not_found_s (Atom "List.find_mapi_or_null_exn: not found") in
  let find_mapi_or_null_exn t ~f =
    match find_mapi_or_null t ~f with
    | Null ->
      raise (Portability_hacks.magic_uncontended__promise_deeply_immutable not_found)
    | This x -> x
  in
  (* named to preserve symbol in compiled binary *)
  find_mapi_or_null_exn
;;
...
[@@kind k = all_ks_non_value] [@@alloc a = heap]
;;

(* call-stack size <= first input data-stack size *)
let%template append =
  let append_stack_tail l1 l2 = exclave_
  ...
  fun [@zero_alloc] l1 l2 ->
    match (l2 : (_ t[@kind k])) with
    | [] -> l1
    | _ :: _ -> exclave_ append_stack_loop l1 l2 ~depth:0
[@@kind k = all_ks] [@@alloc a = stack]
;;
...
let%template filteri_internal t ~f = exclave_
  let rec filteri_stack_tail i xs ~acc = exclave_
    match (xs : (_ t[@kind k])) with
    | [] -> (rev [@kind k] [@alloc stack]) acc
    | x :: xs ->
      let acc : (_ t[@kind k]) = if f #(i, x) then x :: acc else acc in
      (filteri_stack_tail [@tailcall]) (i + 1) xs ~acc
  in
  let rec filteri_stack_loop i xs : (_ t[@kind k]) =
    match (xs : (_ t[@kind k])) with
    | [] -> []
    | x :: xs ->
      exclave_
      ...
      let i = i + 1 in
      (match res with
       | false -> (filteri_stack_loop [@tailcall]) i xs
       | true ->
         let xs =
           if i <= max_non_tailcall
           then filteri_stack_loop i xs
           else (filteri_stack_tail [@inlined never]) i xs ~acc:[]
         in
         x :: xs)
  in
  filteri_stack_loop 0 t
[@@kind k = all_ks] [@@alloc stack]
;;

let%template filteri t ~f = exclave_
  (filteri_internal [@kind k] [@alloc stack] [@inlined]) t ~f:(fun #(i, x) -> f i x)
[@@kind k = all_ks] [@@alloc stack]
;;

(* call-stack size <= output data-stack size *)
let%template[@inline] filter t ~f = exclave_
  (filteri_internal [@kind k] [@alloc a] [@inlined]) t ~f:(fun #(_, x) -> f x)
[@@kind k = all_ks] [@@alloc a = stack]
;;
...
let rev_mapi l ~f =
  (let rec rev_mapi_loop i acc : (_ t[@kind ka]) @ li -> (_ t[@kind kb]) @ lo = function
     | [] -> acc
     | h :: t -> rev_mapi_loop (i + 1) (f i h :: acc) t [@exclave_if_stack a]
   in
   rev_mapi_loop 0 [] l [@nontail])
  [@exclave_if_stack a]
[@@alloc a @ lo = (heap_global, stack_local)]
;;

let mapi_internal l ~f = exclave_
  let rec mapi_stack_tail i xs ~acc : (_ t[@kind kb]) = exclave_
    match (xs : (_ t[@kind ka])) with
    | [] -> (rev [@kind kb] [@alloc a]) acc
    | x :: xs -> (mapi_stack_tail [@tailcall]) (i + 1) xs ~acc:(f #(i, x) :: acc)
  in
  let rec mapi_stack_loop i xs : (_ t[@kind kb]) =
    match (xs : (_ t[@kind ka])) with
    | [] -> []
    | x :: xs ->
      exclave_
      ...
      let ys =
        if i <= max_non_tailcall
        then mapi_stack_loop i xs
        else (mapi_stack_tail [@inlined never]) i xs ~acc:[]
      in
      y :: ys
  in
  mapi_stack_loop 0 l
...
let mapi l ~f = exclave_
  (mapi_internal [@kind ka kb] [@mode li] [@alloc stack] [@inlined])
    l
    ~f:(fun #(i, x) -> exclave_ f i x)
[@@alloc a = stack]
;;

(* call-stack size <= input and output data-stack size *)
let[@inline] map t ~f = exclave_
  (mapi_internal [@kind ka kb] [@mode li] [@alloc a] [@inlined])
    t
    ~f:(fun #(_, x) -> exclave_ f x)
[@@alloc a = stack]
;;

(* call-stack size <= output data-stack size *)
let filter_mapi_internal t ~f = exclave_
  let rec filter_mapi_stack_tail i (xs @ li) ~acc = exclave_
    match (xs : (_ t[@kind ka])) with
    | [] -> (rev [@kind kb] [@alloc a]) acc
    | x :: xs ->
      let acc =
        ...
        | Some y -> y :: acc
      in
      (filter_mapi_stack_tail [@tailcall]) (i + 1) xs ~acc
  in
  let rec filter_mapi_stack_loop i (xs @ li) : (_ t[@kind kb]) =
    match (xs : (_ t[@kind ka])) with
    | [] -> []
    | x :: xs ->
      exclave_
      ...
      (match (y : (_ Option0.t[@kind kb])) with
       | None -> (filter_mapi_stack_loop [@tailcall]) i xs
       | Some y ->
         let ys =
           if i <= max_non_tailcall
           then filter_mapi_stack_loop i xs
           else (filter_mapi_stack_tail [@inlined never]) i xs ~acc:[]
         in
         y :: ys)
  in
  filter_mapi_stack_loop 0 t
[@@alloc a = stack]
;;

let filter_mapi t ~f = exclave_
  (filter_mapi_internal [@kind ka kb] [@mode li] [@alloc stack] [@inlined])
    t
    ~f:(fun #(i, x) -> exclave_ f i x)
[@@alloc a = stack]
;;
...
let filter_map t ~f = exclave_
  (filter_mapi_internal [@kind ka kb] [@mode li] [@alloc a] [@inlined])
    t
    ~f:(fun #(_, x) -> exclave_ f x)
[@@alloc a = stack]
;;

[@@@alloc.default a @ lo = stack_local]
...
let concat_mapi_internal t ~f = exclave_
  let concat_mapi_stack_tail (t : (_ t[@kind ka])) ~i = exclave_
    let #(_, expanded) =
  ...
  in
  let rec concat_mapi_stack_loop xs ~i =
    match (xs : (_ t[@kind ka])) with
    | [] -> ([] : (_ t[@kind kb]))
    | x :: xs ->
      exclave_
      ...
      let expanded_tl =
        if i <= max_non_tailcall
        then concat_mapi_stack_loop xs ~i:(i + 1)
        else (concat_mapi_stack_tail [@inlined never]) xs ~i:(i + 1)
      in
      (append [@kind kb] [@alloc a]) expanded_hd expanded_tl
  in
  concat_mapi_stack_loop t ~i:0
;;

let concat_mapi t ~f = exclave_
  (concat_mapi_internal [@kind ka kb] [@mode li] [@alloc a] [@inlined])
    t
    ~f:(fun #(i, x) -> exclave_ f i x)
;;

let concat_map l ~f = exclave_
  (concat_mapi_internal [@kind ka kb] [@mode li] [@alloc a] [@inlined])
    l
    ~f:(fun #(_, x) -> exclave_ f x)
;;]
...
(* call-stack size <= output data-stack size *)
let filter_mapi_or_null_internal t ~f = exclave_
  let rec filter_mapi_or_null_stack_tail i (xs @ li) ~acc = exclave_
    match (xs : (_ t[@kind ka])) with
    | [] -> (rev [@alloc a]) acc
    | x :: xs ->
      let acc =
        match (f #(i, x) : _ or_null @ local) with
        | Null -> acc
        | This y -> y :: acc
      in
      (filter_mapi_or_null_stack_tail [@tailcall]) (i + 1) xs ~acc
  in
  let rec filter_mapi_or_null_stack_loop i (xs @ li) : _ t =
    match (xs : (_ t[@kind ka])) with
    | [] -> []
    | x :: xs ->
      exclave_
      let y = f #(i, x) in
      let i = i + 1 in
      (match (y : _ or_null @ local) with
       | Null -> (filter_mapi_or_null_stack_loop [@tailcall]) i xs
       | This y ->
         let ys =
           if i <= max_non_tailcall
           then filter_mapi_or_null_stack_loop i xs
           else (filter_mapi_or_null_stack_tail [@inlined never]) i xs ~acc:[]
         in
         y :: ys)
  in
  filter_mapi_or_null_stack_loop 0 t
[@@alloc a = stack]
;;

let filter_mapi_or_null t ~f = exclave_
  (filter_mapi_or_null_internal [@kind ka] [@mode li] [@alloc stack] [@inlined])
    t
    ~f:(fun #(i, x) -> exclave_ f i x)
[@@alloc a = stack]
;;
...
(* call-stack size <= output data-stack size *)
let[@inline] filter_map_or_null t ~f = exclave_
  (filter_mapi_or_null_internal [@kind ka] [@mode li] [@alloc a] [@inlined])
    t
    ~f:(fun #(_, x) -> exclave_ f x)
[@@alloc a = stack]
;;]
...
[%%template
[@@@kind.default ka = all_ks, kb = value_or_null]
...
let mapi l ~f =
  let[@tail_mod_cons] rec local_ mapi_loop i = function
    | [] -> ([] : (_ t[@kind kb]))
...
let map l ~f =
  let[@tail_mod_cons] rec map_loop l : (_ t[@kind kb]) =
    match l with
    | [] -> []
    | x :: tl -> f x :: (map_loop [@tailcall]) tl
  in
  map_loop l [@nontail]
;;

let filter_mapi l ~f =
  let[@tail_mod_cons] rec local_ filter_mapi_loop pos l : (_ t[@kind kb]) =
    match l with
    | [] -> []
    | hd :: tl ->
...
let filter_map l ~f =
  let[@tail_mod_cons] rec filter_map_loop l : (_ t[@kind kb]) =
    match l with
    | [] -> []
    | hd :: tl ->
      (match (f hd : (_ Option0.t[@kind kb])) with
       | None -> filter_map_loop tl
       | Some x -> x :: filter_map_loop tl)
  in
  filter_map_loop l [@nontail]
;;

let concat_mapi l ~(local_ f) =
  let[@tail_mod_cons] rec local_ concat_mapi_outer_loop pos = function
    | [] -> ([] : (_ t[@kind kb]))
    | [ hd ] -> (f [@tailcall false]) pos hd
    | hd :: (_ :: _ as tl) -> concat_mapi_inner_loop (pos + 1) (f pos hd) tl
  and[@tail_mod_cons] local_ concat_mapi_inner_loop pos l1 l2 =
    match l1 with
...
let filter_mapi_or_null l ~f =
  let[@tail_mod_cons] rec local_ filter_mapi_or_null_loop pos (l : (_ t[@kind ka])) : _ t =
    match l with
    | [] -> []
    | hd :: tl ->
...
let filter l ~f =
  let rec local_ filter_loop ~acc l =
    match l with
    | [] -> acc
    | hd :: tl ->
...
let filteri l ~f =
  let rec local_ filteri_loop ~acc pos l =
    match l with
    | [] -> acc
    | hd :: tl ->
...
[%%template
[@@@kind.default ka = all_ks, kb = all_ks_non_value]
[@@@mode.default li = (global, local)]
[@@@alloc.default a = heap]
...
let map (l : (_ t[@kind ka]) @ li) ~(f @ local) : (_ t[@kind kb]) =
  match l with
  | [] -> []
  | [ x ] -> [ f x ]
  | _ :: _ ->
...
let filter_map l ~f =
  let rec local_ filter_map_loop ~acc : (_ t[@kind ka]) @ li -> (_ t[@kind kb]) = function
    | [] -> acc
    | hd :: tl ->
      (match (f hd : (_ Option0.t[@kind kb])) with
...
let filter_mapi l ~f =
  let rec local_ filter_mapi_loop ~acc pos : (_ t[@kind ka]) @ li -> (_ t[@kind kb])
    = function
    | [] -> acc
    | hd :: tl ->
      (match (f pos hd : (_ Option0.t[@kind kb])) with
...
let concat_mapi l ~(local_ f) =
  let rec local_ concat_mapi_loop ~acc pos : (_ t[@kind ka]) @ li -> (_ t[@kind kb])
    = function
    | [] -> []
...
let concat_map l ~(local_ f) =
  let rec local_ concat_map_loop ~acc : (_ t[@kind ka]) @ li -> (_ t[@kind kb]) = function
    | [] -> []
...
let%template fold_map t ~init ~f = exclave_
  let accum, rev_list_accum =
    (fold [@mode li local])
      t
      ~init:(init, [])
      ~f:(fun (accum, rev_list_accum) x -> exclave_
...
(* call-stack size <= input and output data-stack size *)
let map2_ok l1 l2 ~f = exclave_
  let rec map2_ok_stack_tail l1 l2 ~acc = exclave_
    match l1, l2 with
    | [], [] -> (rev [@alloc stack]) acc
    | x1 :: l1, x2 :: l2 -> (map2_ok_stack_tail [@tailcall]) l1 l2 ~acc:(f x1 x2 :: acc)
    | _, _ -> invalid_arg "List.map2"
  in
  let rec map2_ok_stack_loop l1 l2 ~depth =
    match l1, l2 with
    | [], [] -> []
    | x1 :: l1, x2 :: l2 ->
      exclave_
      ...
      let l =
        if depth <= max_non_tailcall
        then map2_ok_stack_loop l1 l2 ~depth:(depth + 1)
        else (map2_ok_stack_tail [@inlined never]) l1 l2 ~acc:[]
      in
      x :: l
    | _, _ -> invalid_arg "List.map2"
  in
  map2_ok_stack_loop l1 l2 ~depth:0
[@@alloc a = stack]
;;
...
let%template unzip list = exclave_
  let rec unzip_stack_tail ~xs ~ys = function
    | [] -> exclave_ #((rev [@alloc stack]) xs, (rev [@alloc stack]) ys)
    | (x, y) :: rest ->
      exclave_ (unzip_stack_tail [@tailcall]) ~xs:(x :: xs) ~ys:(y :: ys) rest
      [@@zero_alloc]
  in
...
let%template fold_mapi t ~init ~f = exclave_
  let accum, rev_list_accum =
    (foldi [@mode li local])
      t
      ~init:(init, [])
      ~f:(fun i (accum, rev_list_accum) x -> exclave_
...
(* We cannot store a [local] accumulator inside of a [ref], so we need a separate
   stack-allocating version *)
let%template groupi l ~break = exclave_
  let groups =
    (foldi [@mode local local]) l ~init:[] ~f:(fun i acc x -> exclave_
      match acc with
      | [] -> [ [ x ] ]
...
let merge_all lists ~compare =
  (* Merge pairs of lists until all are merged *)
  let[@tail_mod_cons] rec merge_pairs ~compare = function
    | [] -> []
    | [ l ] -> [ l ]
    | l1 :: l2 :: rest ->
      (merge [@tailcall false]) l1 l2 ~compare :: (merge_pairs [@tailcall]) ~compare rest
  in
  let rec loop ~compare = function
    | [] -> []
    | [ l ] -> l
    | lists -> loop ~compare (merge_pairs ~compare lists)
  in
  loop ~compare lists
;;
...
let find_all_dups l ~compare =
  ...
  let[@tail_mod_cons] rec local_ find_all_dups_loop sorted prev ~already_recorded =
    match sorted with
    | [] -> []
    | hd :: tl ->
...
let rec all_equal_to t v ~equal =
  match t with
  | [] -> true
  | x :: xs -> equal x v && (all_equal_to [@mode l]) xs v ~equal
;;
...
let all_equal t ~equal =
  ...
  | [] -> None
  | x :: xs ->
    if (all_equal_to [@mode l]) xs x ~equal
    then
      Some x
      [@exclave_if_local l ~reasons:[ May_return_regional; Will_return_unboxed ]]
    else None
...
let rev_filter_map l ~f =
  (let rec rev_filter_map_loop l accum =
     match[@exclave_if_stack a] l with
     | [] -> accum
     | hd :: tl ->
       (match f hd with
        | Some x -> rev_filter_map_loop tl (x :: accum)
        | None -> rev_filter_map_loop tl accum)
   in
   rev_filter_map_loop l [] [@nontail])
  [@exclave_if_stack a]
;;

let rev_filter_mapi l ~f =
  (let rec rev_filter_mapi_loop i l accum =
     match[@exclave_if_stack a] l with
     | [] -> accum
     | hd :: tl ->
       (match f i hd with
        | Some x -> rev_filter_mapi_loop (i + 1) tl (x :: accum)
        | None -> rev_filter_mapi_loop (i + 1) tl accum)
   in
   rev_filter_mapi_loop 0 l [] [@nontail])
  [@exclave_if_stack a]
;;

let rev_filter_map_or_null l ~f =
  (let rec rev_filter_map_or_null_loop l accum =
     match[@exclave_if_stack a] l with
     | [] -> accum
     | hd :: tl ->
       (match f hd with
        | This x -> rev_filter_map_or_null_loop tl (x :: accum)
        | Null -> rev_filter_map_or_null_loop tl accum)
   in
   rev_filter_map_or_null_loop l [] [@nontail])
  [@exclave_if_stack a]
;;

let rev_filter_mapi_or_null l ~f =
  (let rec rev_filter_mapi_or_null_loop i l accum =
     match[@exclave_if_stack a] l with
     | [] -> accum
     | hd :: tl ->
       (match f i hd with
        | This x -> rev_filter_mapi_or_null_loop (i + 1) tl (x :: accum)
        | Null -> rev_filter_mapi_or_null_loop (i + 1) tl accum)
   in
   rev_filter_mapi_or_null_loop 0 l [] [@nontail])
  [@exclave_if_stack a]
;;]
...
let sub l ~pos ~len =
  ...
  let stop = pos + len in
  let[@tail_mod_cons] rec local_ sub_loop i l =
    match l with
    | [] -> []
    | hd :: tl ->
      if i < pos
...
let%template split_n t_orig n =
  ...
  then #([], t_orig)
    ...
    let rec split_n_loop n t accum =
      ...
      | hd :: tl ->
        if n = 0 then #(rev accum, t) else split_n_loop (n - 1) tl (hd :: accum)
    in
...
let%template[@zero_alloc] [@inline] split_n xs n = exclave_
  let rec split_n_stack_tail ~orig ~n ~acc = function
    | [] -> #(orig, [])
    | hd :: tl when n > 0 ->
      exclave_ (split_n_stack_tail [@tailcall]) ~orig ~n:(n - 1) ~acc:(hd :: acc) tl
    | t -> exclave_ #((rev [@alloc stack]) acc, t)
      [@@zero_alloc]
  in
...
(* does not use [split_n] to avoid passing around the dropped part *)
...
let%template take =
  let rec take_stack_tail ~orig ~n ~acc = function
    | [] -> orig
    | hd :: tl when n > 0 ->
      exclave_ (take_stack_tail [@tailcall]) ~orig ~n:(n - 1) ~acc:(hd :: acc) tl
    | _ -> exclave_ (rev [@alloc stack]) acc
  in
  let rec take_stack_loop ~depth ~n = function
    ...
    | _ -> []
  in
  fun [@zero_alloc] t n -> exclave_ take_stack_loop t ~depth:0 ~n
[@@alloc a = stack]
;;
...
(* call stack size is O(input list length)
   ...
   unlike other functions, the maximum call-stack depth is [2 * max_non_tailcall]
*)
let%template[@inline] [@zero_alloc] chunks_of t ~length = exclave_
  let[@cold] rec chunks_of_tail t ~length acc = exclave_
    if is_empty t
  ...
  in
  let rec chunks_of_loop t ~length ~depth = exclave_
    if is_empty t
    then []
...
let chunk_evenly t ~into =
  ...
  let[@tail_mod_cons] rec (chunk_evenly_loop @ local) t ~into =
    match into with
    | 0 -> []
    | _ ->
      let bucket_size =
...
let split_while =
  let[@tail_mod_cons] rec split_while_loop ~f ~rest t =
    match t with
    | hd :: tl when f hd -> hd :: (split_while_loop [@tailcall]) ~f ~rest tl
    | _ ->
      rest := t;
      []
  in
  fun xs ~f ->
    let rest = stack_ (ref []) in
    let first = split_while_loop ~f ~rest xs in
    first, !rest
;;

(* Implemented without [split_while] to avoid allocating a tuple *)
let[@tail_mod_cons] rec take_while xs ~f =
  match xs with
  | hd :: tl when f hd -> hd :: (take_while [@tailcall]) tl ~f
  | _ -> []
;;
...
let drop_last =
  let[@tail_mod_cons] rec loop ~hd:hd1 ~tl =
    match tl with
    | [] -> []
    | hd2 :: tl -> hd1 :: (loop [@tailcall]) ~hd:hd2 ~tl
  in
  function
  | [] -> None
  | hd :: tl -> Some (loop ~hd ~tl)
;;
...
let%template cartesian_product list1 list2 =
  ...
  then []
    ...
    let[@tail_mod_cons] rec local_ cartesian_product_outer_loop l1 =
      match l1 with
      | [] -> []
      | x1 :: l1 -> cartesian_product_inner_loop x1 l1 list2
    and[@tail_mod_cons] local_ cartesian_product_inner_loop x1 l1 l2 =
      match l2 with
...
let%template cartesian_product =
  let rec cartesian_product_stack_tail ~acc l2 = function
    | [] -> exclave_ (rev [@alloc stack]) acc
    | x :: xs -> exclave_ (cartesian_product_stack_inner_tail [@tailcall]) ~acc l2 x xs l2
  and cartesian_product_stack_inner_tail ~acc l2 x xs = function
    | [] -> exclave_ (cartesian_product_stack_tail [@tailcall]) ~acc l2 xs
    | y :: ys ->
      exclave_
  ...
  let rec cartesian_product_stack_loop ~depth l2 = function
    | [] -> exclave_ []
    | x :: xs ->
      exclave_ (cartesian_product_stack_loop_inner [@tailcall]) ~depth l2 x xs l2
  and cartesian_product_stack_loop_inner ~depth l2 x xs = function
    | [] -> exclave_ (cartesian_product_stack_loop [@tailcall]) ~depth l2 xs
    | y :: ys ->
      exclave_
      let rest =
        if depth <= max_non_tailcall
  ...
  in
  fun [@zero_alloc] l1 l2 ->
    if is_empty l2 then [] else exclave_ cartesian_product_stack_loop ~depth:0 l2 l1
[@@alloc a = stack]
;;
...
let equal_with_local_closure (local_ (equal : _ -> _ -> _)) t1 t2 =
  let rec equal_with_local_closure_loop t1 t2 =
    match t1, t2 with
    | [], [] -> true
    | x1 :: t1, x2 :: t2 -> equal x1 x2 && equal_with_local_closure_loop t1 t2
    | _ -> false
  in
  equal_with_local_closure_loop t1 t2 [@nontail]
;;
...
let%template[@mode local] equal equal_a t1 t2 =
  let rec equal_local_loop t1 t2 =
    match t1, t2 with
    | [], [] -> true
    | x1 :: t1, x2 :: t2 -> equal_a x1 x2 && equal_local_loop t1 t2
    | _ -> false
  in
  equal_local_loop t1 t2 [@nontail]
;;
...
let%template fold_until t ~init ~f ~finish =
  (let rec fold_until_loop t ~init =
     match[@exclave_if_local lo ~reasons:[ May_return_local ]] t with
     | [] -> finish init
     | a :: xs ->
       (match f init a with
        | Container.Continue_or_stop.Continue x ->
          (fold_until_loop [@tailcall]) xs ~init:x
        | Stop x -> x)
   in
   fold_until_loop t ~init [@nontail])
  [@exclave_if_local lo ~reasons:[ May_return_local ]]
[@@mode li = (global, local), lo = (global, local)]
;;
...
module%template Local = struct
  module Let_syntax = struct
    ...
    let ( >>| ) x f = exclave_ (map [@mode local] [@alloc stack]) x ~f
    let ( >>= ) x f = exclave_ (concat_map [@mode local] [@alloc stack]) x ~f
