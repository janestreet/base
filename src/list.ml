...
let mapi l ~f =
  let rec loop_tail i xs ~acc ~f : (_ t[@kind kb]) =
...
let map t ~f = (mapi [@kind ka kb] [@mode mi] [@alloc a]) t ~f:(fun _ x -> f x)
[@@mode mi = (global, local)] [@@alloc a = stack]
;;
...
(* call-stack size <= output data-stack size *)
let filter_map t ~f =
  (filter_mapi [@kind ka kb] [@mode mi] [@alloc a]) t ~f:(fun _ x -> f x)
[@@mode mi = (global, local)] [@@alloc a = stack]
;;
...
let filter_mapi l ~f =
  let[@tail_mod_cons] rec loop pos l : (_ t[@kind kb]) =
    match l with
    | [] -> []
    | hd :: tl ->
...
let chunk_evenly t ~into =
  ...
  let[@tail_mod_cons] rec loop t ~into =
    match into with
    | 0 -> []
    | _ ->
      let bucket_size =
