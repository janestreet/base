...
module Local = struct
  ...
  let fold t ~init ~f = exclave_ I.fold_left_local t ~init ~f
  ...
  let foldi t ~init ~f = exclave_ foldi_loop t ~f ~len:(length t) ~pos:0 ~acc:init
...
let foldi t ~init ~(local_ f : _ -> _ -> _ -> _) =
  let n = length t in
