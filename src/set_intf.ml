...
module Definitions = struct
  ...
  module type Accessors_generic = sig
    ...
    val%template equal : ('a, 'cmp, ('a, 'cmp) t -> ('a, 'cmp) t -> bool) access_options
    [@@mode m = (local, global)]
    ...
    val iter2
      : ( 'a
          ...
          , ('a, 'cmp) t
            -> ('a, 'cmp) t
            -> f:
                 ([ `Left of 'a elt | `Right of 'a elt | `Both of 'a elt * 'a elt ]
                  -> unit)
            -> unit )
          access_options
  ...
  module type For_deriving = sig
    ...
    val equal_m__t__local : (module Equal_m) -> ('elt, 'cmp) t -> ('elt, 'cmp) t -> bool
    val globalize_m__t : (module Globalize_m) -> ('elt, 'cmp) t -> ('elt, 'cmp) t
  ...
  module Private = struct
    module type Enum = sig
      ...
      val cons : ('a, 'cmp) tree -> ('a, 'cmp, increasing) t -> ('a, 'cmp, increasing) t
      ...
      val of_set : ('a, 'cmp) tree -> ('a, 'cmp, increasing) t
      val of_set_right : ('a, 'cmp) tree -> ('a, 'cmp, decreasing) t
      ...
      val iter2
        ...
        -> ('a, 'cmp, increasing) t
        -> ('a, 'cmp, increasing) t
        -> f:(('a, 'a) Map.Merge_element.t -> unit)
        -> unit

      (** Traversing one enum *)

      val iter : f:('a -> unit) -> ('a, 'cmp, increasing) t -> unit
    end
  end
end
...
module type Set = sig
  ...
  type (!'elt, !'cmp) t [@@deriving compare ~localize, globalize]
  ...
  val%template compare_direct : ('a, 'cmp) t -> ('a, 'cmp) t -> int
  [@@mode m = (local, global)]
  ...
  val%template equal : ('a, 'cmp) t -> ('a, 'cmp) t -> bool
  [@@mode m = (local, global)]
