...
module Definitions = struct
  module type S = sig
    (** Floating-point representation and utilities.
        ...
        If using 32-bit OCaml, you cannot quite assume operations act as you'd expect for
        IEEE 64-bit floats. E.g., one can have [let x = ~-. (2. ** 62.) in x = x -. 1.]
        evaluate to [false] while [let x = ~-. (2. ** 62.) in let y = x -. 1 in x = y]
        evaluates to [true]. This is related to 80-bit registers being used for
        calculations; you can force representation as a 64-bit value by let-binding. *)
    ...
    (** An order-preserving bijection between all floats except for nans, and all int64s
        with absolute value smaller than or equal to [2**63 - 2**52]. Note both 0. and -0.
        map to 0L. *)
    ...
    (** The next or previous representable float. ULP stands for "unit of least
        precision", and is the spacing between floating point numbers. Both
        [one_ulp `Up infinity] and [one_ulp `Down neg_infinity] return a nan. *)
    ...
    (** [round] rounds a float to an integer float. [iround{,_exn}] rounds a float to an
        int. Both round according to a direction [dir], with default [dir] being
        [`Nearest].
      ...
      | `Zero    | rounds toward zero                                           |
        ...
        [iround_exn] raises when trying to handle nan or trying to handle a float outside
        the range \[float min_int, float max_int).
      ...
      | `Nearest | [-1.5,-0.5) to -1. | [-0.5,0.5) to 0.  | [0.5,1.5) to 1.              |
        ...
        For convenience, versions of these functions with the [dir] argument hard-coded
        are provided. If you are writing performance-critical code you should use the
    ...
    val round : ?dir:local_ [ `Zero | `Nearest | `Up | `Down ] -> local_ t -> t

    val iround : ?dir:local_ [ `Zero | `Nearest | `Up | `Down ] -> local_ t -> int option

    val iround_exn : ?dir:local_ [ `Zero | `Nearest | `Up | `Down ] -> local_ t -> int
    [@@zero_alloc]
    ...
    (** [round_significant x ~significant_digits:n] rounds to the nearest number with [n]
        ...
        [sprintf "%.*g" n x |> Float.of_string] but faster (10x-15x). Exact ties are
        resolved as round-to-even.
    ...
    val round_significant : t @ m -> significant_digits:int -> t @ m

    (** [round_decimal x ~decimal_digits:n] rounds [x] to the nearest [10**(-n)]. For
        positive [n] it is meant to be equivalent to
        [sprintf "%.*f" n x |> Float.of_string], but faster.

        All the considerations mentioned in [round_significant] apply (both functions use
        the same code path). *)
    val round_decimal : t @ m -> decimal_digits:int -> t @ m]
    ...
    (** [mod_float x y] returns a result with the same sign as [x]. It returns [nan] if
        [y] is [0]. It is basically
    ...
    (** Similar to [O], except that operators are suffixed with a dot, allowing one to
        have both int and float operators in scope simultaneously. *)
    module O_dot : sig
      external ( +. )
        :  (t[@local_opt])
        -> (t[@local_opt])
    ...
    (** [to_string x] builds a string [s] representing the float [x] that guarantees the
        round trip, that is such that [Float.equal x (Float.of_string s)].
    ...
    (** Produce a lossy compact string representation of the float. The float is scaled by
        ...
        The smallest scale factor that allows the number to be rendered with at most 3
        digits to the left of the decimal is used. If the number is too large for this
        format (i.e., the absolute value is at least 999.95e15), scientific notation is
        used instead. E.g.:
        ...
        In the case where the digit after the "decimal", or the "decimal" itself is
        ...
        [to_padded_compact_string] is defined in terms of
        [to_padded_compact_string_custom] below as
        {[
    ...
    (** [int_pow x n] computes [x ** float n] via repeated squaring. It is generally much
        ...
        Note that [int_pow x 0] always returns [1.], even if [x = nan]. This coincides
        with [x ** 0.] and is intentional.
        ...
        words. Even if called in a way that causes allocation, it still appears to be
        faster than [**]. *)
    ...
    (** [frexp f] returns the pair of the significant and the exponent of [f]. When [f] is
        zero, the significant [x] and the exponent [n] of [f] are equal to zero. When [f]
    ...
    (** [expm1 x] computes [exp x -. 1.0], giving numerically-accurate results even if [x]
        is close to [0.0]. *)
    ...
    (** [copysign x y] returns a float whose absolute value is that of [x] and whose sign
        is that of [y]. If [x] is [nan], returns [nan]. If [y] is [nan], returns either
        [x] or [-. x], but it is not specified which. *)
    external copysign
      :  (t[@local_opt])
      -> (t[@local_opt])
      -> t
    ...
    (** [atan2 y x] returns the arc tangent of [y /. x]. The signs of [x] and [y] are used
        to determine the quadrant of the result. Result is in radians and is between [-pi]
        and [pi]. *)
    ...
    (** [hypot x y] returns [sqrt(x *. x + y *. y)], that is, the length of the hypotenuse
        of a right-angled triangle with sides of length [x] and [y], or, equivalently, the
    ...
    (** Hyperbolic arc sine. The argument and result range over the entire real line.
    ...
    (** The sign of a float, with support for NaN. Both [-0.] and [0.] map to [Zero]. All
        NaN values map to [Nan]. All other values map to [Neg] or [Pos]. *)
    val sign_or_nan : local_ t -> Sign_or_nan.t

    (** These functions construct and destruct 64-bit floating point numbers based on
        their IEEE representation with a sign bit, an 11-bit non-negative (biased)
        exponent, and a 52-bit non-negative mantissa (or significand). See
        {{:http://en.wikipedia.org/wiki/Double-precision_floating-point_format} Wikipedia}
...
module type Float = sig @@ portable
