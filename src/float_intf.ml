...
module type S = sig
  ...
  (** Produce a lossy compact string representation of the float. The float is scaled by
      ...
      format (i.e., the absolute value is at least 999.95e15), scientific notation is used
      instead. E.g.:
      ...
      the numbers are padded on the right with spaces to ensure the last two columns of
      the string always correspond to the decimal and the digit afterward (except in the
      case of scientific notation, where the exponent is the right-most element in the
      string and could take up to four characters).
  ...
  (** [int_pow x n] computes [x ** float n] via repeated squaring. It is generally much
      ...
      words. Even if called in a way that causes allocation, it still appears to be faster
      than [**]. *)
  ...
  (** Hyperbolic arc cosine. The argument must fall within the range [[1.0, inf]]. Result
      is in radians and is between [0.0] and [inf]. *)
