...
(** [from_fun f] is the same as [lazy (f ())] but slightly more efficient if [f] is a
    ...
    particular it is always less efficient to write [from_fun (fun () -> expr)] than
    [lazy expr]. *)
val from_fun : (unit -> 'a) -> 'a t

(** [from_val v] returns an already-forced suspension of [v] (where [v] can be any
    expression). Essentially, [from_val expr] is the same as [let var = expr in lazy var]. *)
val from_val : 'a -> 'a t
...
(** [peek x] returns None if [x] has never been forced or [Some v] if [x] was forced to
    value [v] *)
