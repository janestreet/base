...
(* The default size is chosen to be 0 (as opposed to 128 as it was before) because:
   ...
   - in other languages (like rust, python, and apparently go), the default is also a
     small size. *)
let create (type v) ?(growth_allowed = true) ?(size = 0) ~hashable ()
  : ((_, v) t[@kind k v])
  =
...
let add_worker (type k v) (t : ((k, v) t[@kind k v])) ~replace ~key ~data =
...
let add (type k v) (t : ((k, v) t[@kind k v])) ~key ~data =
  (ensure_mutation_allowed [@kind k v]) t;
...
let choose_nonempty (type k v) (table : ((k, v) Avltree.t[@kind k v]) array) idx : k * v =
  let rec loop i =
...
module KV = struct
  ...
  external to_pairs : 'k 'v. ('k, 'v) t list -> ('k * 'v) list = "%identity"
end
...
let sexp_of_t (type k v) sexp_of_key sexp_of_data (t : ((k, v) t[@kind k v])) : Sexp.t =
  t
  |> (to_alist [@kind k v])
...
type ('key, 'value) filter_map_item =
  { key : 'key
...
let mapi_inplace (type k v) (t : ((k, v) t[@kind k v])) ~f =
  (ensure_mutation_allowed [@kind k v]) t;
