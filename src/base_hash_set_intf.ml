...
module type Accessors = sig
  ...
  val of_hashtbl_keys : ('a, _) Base_hashtbl.t -> 'a t
  val to_hashtbl : 'key t -> f:('key -> 'data) -> ('key, 'data) Base_hashtbl.t
end

type ('key, 'z) create_options_without_hashable =
  ('key, unit, 'z) Base_hashtbl_intf.create_options_without_hashable

type ('key, 'z) create_options_with_hashable_required =
  ('key, unit, 'z) Base_hashtbl_intf.create_options_with_hashable

type ('key, 'z) create_options_with_first_class_module =
  ('key, unit, 'z) Base_hashtbl_intf.create_options_with_first_class_module
