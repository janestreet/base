...
type%template 'a t = 'a array [@@kind k = (base_non_value, value_or_null mod external64)]
...
struct
  ...
  module Intro_sort : sig
    ...
    let five_element_sort (type a) (arr : a t) ~(compare : _ -> _ -> _) m1 m2 m3 m4 m5 =
    ...
    let choose_pivots (type a) (arr : a t) ~(compare : _ -> _ -> _) ~left ~right =
    ...
    let dual_pivot_partition (type a) (arr : a t) ~(compare : _ -> _ -> _) ~left ~right =
    ...
    let rec intro_sort
      : type a. a t -> max_depth:_ -> compare:(a -> a -> int) -> left:_ -> right:_ -> _
      =
    ...
    let sort (type a) (arr : a t) ~compare ~left ~right =
  ...
  let sort (type a) ?pos ?len (arr : a t) ~compare =
    let pos, len =
...
let[@inline] is_sorted_internal t ~in_order =
  let n = length t in
  match n < 2 with
  | true -> true
  | false ->
...
let of_list_rev (l : (_ List.Constructors.t[@kind k1 or value_or_null])) : _ t =
  match l with
  | [] -> [||]
  | a :: l ->
...
let sum (type sum) (module M : Container.Summable with type t = sum[@kind k2]) t ~f =
...
let of_list_map (xs : (_ List.Constructors.t[@kind k1 or value_or_null])) ~f : _ t =
  match xs with
  | [] -> [||]
  | hd :: tl ->
    let a =
...
let of_list_mapi (xs : (_ List.Constructors.t[@kind k1 or value_or_null])) ~f : _ t =
  match xs with
  | [] -> [||]
  | hd :: tl ->
    let a =
...
let merge (type a) (a1 : a t) a2 ~compare =
