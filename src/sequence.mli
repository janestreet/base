...
(** A sequence of elements that can be produced one at a time, on demand, normally with no
    ...
    the sequence. These functions will always return in O(1), but traversing the resulting
    sequence may be more expensive. The most they will do immediately is generate a new
    internal state and a new step function.
    ...
    [drop 1] n times will cost O(n) per element traversed in the result), there are also
    more eager versions of many functions (whose names are suffixed with [_eagerly]) that
    do more work up front. A function has the [_eagerly] suffix iff it matches both of
    these conditions:
...
(** [next] returns the next element of a sequence and the next tail if the sequence is not
    finished. *)
val next : ('a : value_or_null). 'a t -> ('a * 'a t) option
...
val unfold_with_and_finish
  : ('a : value_or_null) ('b : value_or_null) ('s_b : value_or_null)
    ('s_a : value_or_null).
  'a t
  -> init:'s_a
...
val nth : ('a : value_or_null). 'a t -> int -> 'a option

val nth_exn : ('a : value_or_null). 'a t -> int -> 'a
...
val folding_map
  ...
  'a t -> init:'acc -> f:('acc -> 'a -> 'acc * 'b) -> 'b t

val folding_mapi
  : ('a : value_or_null) ('b : value_or_null) ('acc : value_or_null).
  'a t -> init:'acc -> f:(int -> 'acc -> 'a -> 'acc * 'b) -> 'b t

val mapi : ('a : value_or_null) ('b : value_or_null). 'a t -> f:(int -> 'a -> 'b) -> 'b t
val filteri : ('a : value_or_null). 'a t -> f:(int -> 'a -> bool) -> 'a t
val filter : ('a : value_or_null). 'a t -> f:('a -> bool) -> 'a t
...
(** If [t1] and [t2] are each sorted without duplicates,
    ...
    one from [t1] is used and the one from [t2] is discarded. The behavior is undefined if
    the inputs aren't sorted or contain duplicates. *)
...
(** If [t1] and [t2] are each sorted, [merge_sorted t1 t2 ~compare] merges [t1] and [t2]
    ...
    sorted. *)
val merge_sorted : ('a : value_or_null). 'a t -> 'a t -> compare:('a -> 'a -> int) -> 'a t
...
val merge_with_duplicates
  ...
  'a t -> 'b t -> compare:('a -> 'b -> int) -> ('a, 'b) Merge_with_duplicates_element.t t
...
val tl_eagerly_exn : ('a : value_or_null). 'a t -> 'a t
...
val concat_map : ('a : value_or_null) ('b : value_or_null). 'a t -> f:('a -> 'b t) -> 'b t
...
(** [interleave tt] produces each element of the inner sequences of [tt] eventually, even
    ...
    are produced in order with respect to that inner sequence. The manner of interleaving
    among the separate inner sequences is deterministic but unspecified. *)
val interleave : ('a : value_or_null). 'a t t -> 'a t
...
val round_robin : ('a : value_or_null). 'a t list -> 'a t
...
val zip : ('a : value_or_null) ('b : value_or_null). 'a t -> 'b t -> ('a * 'b) t
...
val zip_full
  : ('a : value_or_null) ('b : value_or_null).
  'a t -> 'b t -> [ `Left of 'a | `Both of 'a * 'b | `Right of 'b ] t
...
val group : ('a : value_or_null). 'a t -> break:('a -> 'a -> bool) -> 'a list t
...
(** The same sequence with consecutive duplicates removed. The relative order of the other
...
val init : ('a : value_or_null). int -> f:(int -> 'a) -> 'a t
...
val drop_eagerly : ('a : value_or_null). 'a t -> int -> 'a t

(** [take_while t ~f] produces the longest prefix of [t] for which [f] applied to each
    element is [true]. *)
val take_while : ('a : value_or_null). 'a t -> f:('a -> bool) -> 'a t
...
val drop_while : ('a : value_or_null). 'a t -> f:('a -> bool) -> 'a t
...
val drop_while_option
  : ('a : value_or_null).
  'a t -> f:local_ ('a -> bool) -> ('a * 'a t) option
...
val split_n : ('a : value_or_null). 'a t -> int -> 'a list * 'a t
...
val chunks_exn : ('a : value_or_null). 'a t -> int -> 'a list t

(** [shift_right t a] produces [a] and then produces each element of [t]. *)
val shift_right : ('a : value_or_null). 'a t -> 'a -> 'a t
...
val shift_right_with_list : ('a : value_or_null). 'a t -> 'a list -> 'a t

(** [shift_left t n] is a synonym for [drop t n]. *)
val shift_left : ('a : value_or_null). 'a t -> int -> 'a t
...
val cycle_list_exn : ('a : value_or_null). 'a list -> 'a t
...
val delayed_fold
  : ('a : value_or_null) ('s : value_or_null) ('r : value_or_null).
  'a t
  -> init:'s
...
val fold_m
  : ('elt : value_or_null) ('acc : value_or_null) ('acc_m : value_or_null).
...
(** [iter_m] is a monad-friendly version of [iter]. Supply it with the monad's [return]
    and [bind], and it will chain them through the computation. *)
val iter_m
  : ('elt : value_or_null) ('unit_m : value_or_null).
...
(** [force_eagerly t] precomputes the sequence. It is behaviorally equivalent to
    [of_list (to_list t)], but may at some point have a more efficient implementation.
    It's an eager version of [memoize]. *)
val force_eagerly : ('a : value_or_null). 'a t -> 'a t

(** [bounded_length ~at_most t] returns [`Is len] if [len = length t <= at_most], and
    otherwise returns [`Greater]. Walks through only as much of the sequence as necessary.
    Always returns [`Greater] if [at_most < 0]. *)
val bounded_length
  : ('a : value_or_null).
  'a t -> at_most:int -> [ `Is of int | `Greater ]

(** [length_is_bounded_by ~min ~max t] returns true if [min <= length t] and
    [length t <= max] When [min] or [max] are not provided, the check for that bound is
    omitted. Walks through only as much of the sequence as necessary. *)
val length_is_bounded_by : ('a : value_or_null). ?min:int -> ?max:int -> 'a t -> bool

val of_seq : ('a : value_or_null). 'a Stdlib.Seq.t -> 'a t
val to_seq : ('a : value_or_null). 'a t -> 'a Stdlib.Seq.t
    ...
    {[
      ...
      let rec traverse_bst = function
        ...
        | Node.Branch (left, value, right) ->
          traverse_bst left >>= fun () -> yield value >>= fun () -> traverse_bst right
      ;;
...
(** The functions in [Expert] expose internal structure which is normally meant to be
    ...
    {[
      List.filter xs ~f |> Sequence.of_list
    ]}

    and

    {[
      Sequence.of_list xs |> Sequence.filter ~f
    ]}
