...
module T = struct
  type t = nativeint [@@deriving hash, sexp]
...
let of_float f =
  if f >= float_lower_bound && f <= float_upper_bound then
    ...
    Base_printf.invalid_argf "Nativeint.of_float: argument (%f) is out of range or NaN"
      (Float0.box f)
      ()
...
include Conv.Make_hex(struct
  ...
  type t = nativeint [@@deriving compare, hash]
