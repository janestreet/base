open! Import
open! Base_hash_set_intf

module List = StdLabels.List

module Hashable = Base_hashtbl.Hashable
module Hashtbl  = Base_hashtbl.Using_hashable
...
module Accessors = struct
  ...
  let hashable = Base_hashtbl.hashable
...
module Poly = struct
  ...
  let hashable = Base_hashtbl.Poly.hashable
...
module type M_of_sexp = sig
  ...
  include Base_hashtbl_intf.Key_plain with type t := t
end
...
let%test_module "Set Intersection" = (module struct
  ...
  let run_test first_contents second_contents ~expect =
    ...
    assert (Base_list.equal ~equal (to_list result) (to_list expect));
