...
let rec linear_search_first_satisfying (t @ m) ~get ~lo ~hi ~pred = exclave_
  if lo > hi
  then None
...
let find_first_satisfying ?pos ?len (t @ m) ~get ~length ~pred = exclave_
  let pos, len =
...
let binary_search
  ...
  (t @ m)
  ~(local_ length : _ @ m -> _)
  ~(local_ get : _ @ m -> _ -> _ @ m)
  ~(local_ compare : _ @ m -> _ @ m -> _)
  how
  (v @ m)
  = exclave_
  match how with
...
let binary_search_segmented ?pos ?len (t @ m) ~length ~get ~segment_of how = exclave_
  let is_left x =
