    ...
    Sexplib.Conv.Exn_converter.add
      ...
      (function
        | Set_min_elt_exn_of_empty_set  ->
          Sexplib.Sexp.Atom "src/set.ml.Tree0.Set_min_elt_exn_of_empty_set"
        | _ -> assert false)
    ...
    Sexplib.Conv.Exn_converter.add
      ...
      (function
        | Set_max_elt_exn_of_empty_set  ->
          Sexplib.Sexp.Atom "src/set.ml.Tree0.Set_max_elt_exn_of_empty_set"
        | _ -> assert false)
  ...
  let is_subset s1 ~of_:s2 ~compare_elt =
    let rec is_subset s1 ~of_:s2 =
      ...
      | Node (l1, v1, r1, _, _), (Node (l2, v2, r2, _, _) as t2) ->
        let c = compare_elt v1 v2 in
        if c = 0
        then is_subset l1 ~of_:l2 && is_subset r1 ~of_:r2
        (* Note that height and size don't matter here. *)
        else if c < 0 then
          is_subset (Node (l1, v1, Empty, 0, 0)) ~of_:l2 && is_subset r1 ~of_:t2
        else
          is_subset (Node (Empty, v1, r1, 0, 0)) ~of_:r2 && is_subset l1 ~of_:t2
    in
    is_subset s1 ~of_:s2
  ;;
...
let stable_dedup_list m a = Using_comparator.stable_dedup_list ~comparator:(to_comparator m) a
...
module M (Elt : sig type t type comparator_witness end) = struct
...
let compare_m__t (module Elt : Compare_m) t1 t2 =
