...
module Tree0 = struct
  ...
  let%test _ =
    let map = of_increasing_iterator_unchecked ~len:20 ~f:(fun x -> x,x) in
    let compare_key x y =
      if x = y      then 0
      else if x < y then -1
      else               1
  ...
  let merge t1 t2 ~f ~compare_key =
    let elts = Base_array.create ~len:(length t1 + length t2) (Obj.magic None) in
    let i = ref 0 in
    iter2 t1 t2 ~compare_key ~f:(fun ~key ~data:values ->
      match f ~key values with
      | Some value -> elts.(!i) <- (key, value); incr i
      | None -> ());
    (* [Base_array.unsafe_truncate] raises if [len = 0] *)
    if !i = 0 then
      (empty, 0)
    else begin
      Base_array.unsafe_truncate elts ~len:!i;
...
module Tree = struct
  ...
  let nth_exn ~comparator t n = nth ~comparator t n |> Option.value_exn
...
module type M_of_sexp = sig
  type t [@@deriving of_sexp] include Comparator.S with type t := t
end
module type Compare_m = sig end
module type Hash_fold_m = Hasher.S

let sexp_of_m__t (type k) (module K : Sexp_of_m with type t = k) sexp_of_v t =
  sexp_of_t K.sexp_of_t sexp_of_v [%of_sexp: _] t
