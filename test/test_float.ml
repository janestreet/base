...
module%test [@name "clamp"] _ = struct
  ...
  let%test_unit "clamp_exn bad" =
    Expect_test_helpers_base.require_does_raise (fun () -> clamp_exn 2.5 ~min:3. ~max:2.)
  ;;

  let%test_unit "clamp_exn also bad" =
    Expect_test_helpers_base.require_does_raise (fun () -> clamp_exn 2.5 ~min:nan ~max:3.)
  ;;

  let%test_unit "clamp_exn also bad 2" =
    Expect_test_helpers_base.require_does_raise (fun () -> clamp_exn 2.5 ~min:2. ~max:nan)
  ;;
...
module%test _ = struct
  ...
  *)
  ...
  let%test_unit (_ [@tags "no-js"]) = boundary (incr 0.25) ~closer_to_zero:"0.2" ~at:"0.3"
  ...
  let%test_unit (_ [@tags "no-js"]) = boundary (incr 3.25) ~closer_to_zero:"3.2" ~at:"3.3"
...
module%test Int = Test_bounds (Int)
module%test Int32 = Test_bounds (Int32)
module%test Int63 = Test_bounds (Int63)
module%test Int63_emul = Test_bounds (Base.Int63.Private.Emul)
module%test Int64 = Test_bounds (Int64)
module%test Nativeint = Test_bounds (Nativeint)
...
module%test [@name "Hexadecimal syntax"] _ = struct
...
let%expect_test "float comparisons permit both local and global arguments" =
  ...
  let (_ : local_ float -> local_ float -> bool) = Float.( < ) in
  [%expect {| |}]
;;
