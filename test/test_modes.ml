...
module _ : Modes.At_locality.Without_crossing = struct
  ...
  type (+'a, 'locality) t =
    | Local : 'a -> ('a, local) t
    | Global : ('a[@globalized]) -> ('a, _) t

  let wrap a = Global a
  let unwrap (type locality) (Local a | Global a : (_, locality) t) = a
  let wrap_local a = Local a
  let unwrap_local (type locality) (Local a | Global a : (_, locality) t) = a
...
module M : sig
  ...
  let return_at_max : string -> t (* unique *) =
