...
external get : ('a array[@local]) -> int -> 'a = "%array_safe_get"
external set : ('a array[@local]) -> int -> 'a -> unit = "%array_safe_set"

let%expect_test "no sharing between globalized arrays" =
  let a = [%local] [| "initial" |] in
