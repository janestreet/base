...
module%test _ : module type of Queue = struct
  open Queue
  ...
  module%test [@name "comparisons"] _ = struct
    ...
    let test t1 t2 =
      ...
      [%test_result: int]
        ...
        ~expect:(sign (List.compare__local Int.compare__local (to_list t1) (to_list t2)))
    ;;
  ...
  module%test [@name "Linked_queue bisimulation"] _ = struct
    module type Queue_intf = sig
      ...
      val foldi_until
        :  'a t
        -> init:'b
        -> f:(int -> 'b -> 'a -> ('b, 'c) Continue_or_stop.t)
        -> finish:(int -> 'b -> 'c)
        -> 'c
      ...
      val iteri_until
        :  'a t
        -> f:(int -> 'a -> (unit, 'b) Continue_or_stop.t)
        -> finish:(int -> 'b)
        -> 'b
