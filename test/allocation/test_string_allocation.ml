...
module%test [@name "common prefix and suffix"] _ = struct
    ...
    Staged.stage (fun list ->
      ...
      then print_endline "(may allocate)"
        ...
        Option.iter (get_shortest_and_longest list) ~f:(fun (shortest, longest) ->
          ignore (require_no_allocation (fun () -> get_common2 shortest longest) : string);
          ignore (require_no_allocation (fun () -> get_common2 longest shortest) : string))))
  ;;
