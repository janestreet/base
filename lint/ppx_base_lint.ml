...
let check_open (id : Longident.t Asttypes.loc) =
  ...
  | Lident "Caml" ->
    error ~loc:id.loc
      "you are not allowed to open Caml inside Base"
  | _ -> ()
      ...
      iter_suspicious id ~f:(function
        | Caml_submodule m ->
          if not (Set.mem zero_modules m) then
            ...
            | _ ->
              error ~loc
                "you cannot use [Caml.%s] here, use [%s0] instead" m m)
    ...
    method! module_binding mb =
      ...
      | _ ->
        ...
        | Pmod_ident { txt = id; _ } when is_caml_dot_something id ->
          error ~loc:mb.pmb_loc
            "you cannot alias [Caml] sub-modules, use them directly"
        | _ -> ()
  end
